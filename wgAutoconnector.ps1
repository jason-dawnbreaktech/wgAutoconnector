...
function Is-VPNConnected {
   try {
      ...
      Write-Host "The VPN knows where it is. It knows this because it knows where it isn't."
      Write-Host "Currently connected to: $currentSSID"
      return $true
   } else {
      Write-Host "The VPN does not know where it is. This may be because it is where it wasn't. Variance detected. Recalibrating."
      return $false
      }
   }
   catch {
...
function Get-CurrentSSID {
    ...
    Write-Host "You gotta connect to the internet yo."
    return $null
    }
}
